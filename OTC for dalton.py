...
bot = Bot(token="8598686510:AAHnF7AZ4pH7SfKSH13meH1BM38o9VElAxY")
dp = Dispatcher()
all_users = set() 
...
main_keyboard_ru = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="📞 Поддержка", url="https://t.me/PlayrokOTCsupport")],
    ...
    if deal_id:
        ...
        if deal["type"] == "deal_gift":
            text_ru = (
                ...
                f"NFT ожидает отправки на официальный аккаунт менеджера - @PlayrokOTCsupport\n\n"
            ...
            text_en = (
                ...
                f"NFT must be sent to the official manager account — @PlayrokOTCsupport\n\n"
                f"⚠️ Attention:\n"
...
async def sierrateam_read_callback(callback: CallbackQuery):
    ...
    await safe_edit_message(
        callback,
        "👑 Админ-панель\n\nВыберите действие:\n\n🔓 Полный доступ: ❌ Отсутствует\n💼 Может подтверждать: Только подарки\n\n💎 Для получения полного доступа свяжитесь с @PlayrokOTCsupport",
...
async def agree_callback(callback: CallbackQuery):
    ...
    lang = user_languages.get(user_id, "ru")
    text = "Добро пожаловать в Playerok!\nПоддержка - @PlayrokOTCsupport" if lang == "ru" else "Welcome to Playerok!\nSupport - @PlayrokOTCsupport"
